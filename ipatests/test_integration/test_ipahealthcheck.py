...
class TestIpaHealthCheckWithExternalCA(IntegrationTest):
    ...
    @pytest.fixture()
    def rename_ipa_cert(self):
        """
        Fixture to rename ipa cert and revert the change.
        """
        self.master.run_command(
            ["mv", paths.IPA_CA_CRT, "%s.old" % paths.IPA_CA_CRT]
        )
        yield
        self.master.run_command(
    ...
    def test_opensslchainvalidation_ipa_ca_cert(self, rename_ipa_cert):
        """
        Test for IPAOpenSSLChainValidation when IPA CA cert is renamed
        """
