...
class Testmaster(object):
    ...
    def test_0005_bz1283890(self, multihost):
        ...
        Testcase to validate IPA server install parameters
        """
        exp_output = "ipa-server-install: error: --subject option requires 1 argument"
        multihost.master.qerun(['ipa-server-install', '--setup-dns',
    ...
    def test_0006_bz1283890(self, multihost):
        ...
        cmdline = ['ipa-server-install', '--setup-dns', '--forwarder=10.11.5.19', '-r', 'TESTRELM.TEST', '-p',
                   'password', '-a', 'password', '--subject=NOSUBJECT', '-U']
        print(" ".join(cmdline))
        multihost.master.qerun(cmdline, exp_returncode=2, exp_output=exp_output)
    ...
    def test_0006_bz1283890_with_subject_base(self, multihost):
        ...
        cmdline = ['ipa-server-install', '--setup-dns', '--forwarder=10.11.5.19', '-r', 'TESTRELM.TEST', '-p',
                   'password', '-a', 'password', '--subject-base=NOSUBJECT', '-U']
        print(" ".join(cmdline))
        multihost.master.qerun(cmdline, exp_returncode=2, exp_output=exp_output)
