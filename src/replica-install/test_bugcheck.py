...
class Testmaster(object):
    ...
    def class_setup(self, multihost):
        ...
        output2 = multihost.replica.run_command(['rpm', '-q', 'ipa-server'],
                                                set_env=False, raiseonerr=False)
        if output2.returncode != 0:
            print("IPA server package not found on REPLICA, thus installing")
            install1 = multihost.replica.run_command(['yum', 'install', '-y', 'ipa-server*'])
            if install1.returncode == 0:
                print("IPA server package installed.")
            else:
                pytest.xfail("IPA server package installation failed, check repo links for further debugging")
        else:
            print("\n IPA server package found on REPLICA, running tests")

    def test_0001_ipareplicainstall(self, multihost):
        multihost.replica.qerun(['ipa-replica-install', '--forwarder=a.c.b.d'],
                                exp_returncode=2,
                                exp_output="ipa-replica-install: error: option --forwarder:")

    def test_0002_ipareplicainstall(self, multihost):
        multihost.replica.qerun(['ipa-replica-install'],
                                exp_returncode=2,
                                exp_output="ipa-replica-install: error: you must provide a file generated by ipa-replica-prepare")
