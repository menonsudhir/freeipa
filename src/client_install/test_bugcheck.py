...
class TestBugCheck(object):
    ...
    def test_0001_bz1196656_bz1284025(self, multihost):
        ...
        if multihost.master.transport.file_exists('/etc/ipa/default.conf'):
            ...
            if check2.returncode == 0:
                check3 = multihost.client.qerun('tail -n 3 /var/log/ipaclient-install.log',
                                                exp_output='complete')
                check4 = multihost.client.qerun('tail -5 /etc/ssh/sshd_config',
                                                exp_output='10.65.207.112')
                if check3.returncode and check4.returncode == 0:
                    print('Client installation complete, BZ1196656 and BZ1284025 PASSED')
                else:
                    pytest.xfail('BZ1196656 or BZ1284025 FAILED,client did not install')
        else:
            pytest.xfail("IPA client is not installed, BZ1196656 FAILED")

    def test_0001_bz1215200_bz1211708(self, multihost):
        ...
        if check5.returncode != 0:
            ...
            setup_client(multihost.client, multihost.master)
            check6 = multihost.client.run_command('ls /etc/ipa/ | grep default.conf',
                                                  raiseonerr=False)
    ...
    def test_0001_bz1215197(self, multihost):
